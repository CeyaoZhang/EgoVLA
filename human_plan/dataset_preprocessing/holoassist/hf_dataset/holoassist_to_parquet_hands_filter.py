...
def holoassist_write_parquet(
  ...
  for seq_name in tqdm.tqdm(all_seqs, desc=f"{chunk_id} out of {num_chunks}"):
    seq_data = parse_single_seq_hand_only(
      ...
      future_len=future_len * frame_skip,
    ...
    if seq_idx % write_frequency == 0:
      save_data_to_parquet(
        ...
        skip_keys=["rgb_obs", 
            "language_label", "language_label_short", "language_label_empty",
            "language_label_verb", "language_label_noun", "language_label_adj", "language_label_adv",
            "language_response",  
            "frame_count", "seq_name", "raw_width", "raw_height", 
        ]
      )
      save_idx += 1
      seq_data_list = []
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--future_len", type=int, default=61,
    help="Number of parallel workers"
  )
  ...
  # Example usage
  dataset_root = "data/ha_dataset/HoloAssist"
  save_path = f"data/ha_dataset/hand_filtered{args.additional_tag}_{set_label}_parquets"
  ...
  image_mapping_path = "data/ha_dataset/hf_images_v2_mapping.pkl"
  ...
  all_seqs = get_all_seqs(os.path.join(dataset_root, "raw_data"))
# Open the file and read the lines
  ...
  holoassist_write_parquet(
    all_seqs=all_seqs,
    dataset_root=os.path.join(dataset_root, "raw_data"),
    annotations=annotations,
