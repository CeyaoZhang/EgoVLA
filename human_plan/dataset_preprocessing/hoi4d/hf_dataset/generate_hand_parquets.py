...
def hoi4d_write_parquet(
  ...
  hand_zip_path = os.path.join(
    dataset_root, "handpose.zip"
  )
  annotation_zip_path = os.path.join(
    dataset_root, "HOI4D_annotations.zip"
  )

  with zipfile.ZipFile(hand_zip_path, 'r') as hand_pose_zip_ref:
    with zipfile.ZipFile(annotation_zip_path, 'r') as annotation_zip_ref:
      for seq_name in tqdm.tqdm(
        all_seqs,
        desc=f"{chunk_id} out of {num_chunks}"
      ):
        seq_data = parse_single_seq_hand(
          hand_pose_zip_ref=hand_pose_zip_ref,
          annotation_zip_ref=annotation_zip_ref,
          hand_zip_file_list=hand_pose_zip_ref.namelist(),
          cam_zip_file_list=annotation_zip_ref.namelist(),
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--frame_repeat", type=int, default=1,
    help="Frame Skip"
  )
  ...
  # Example usage
  dataset_root = "data/HOI4D"
  save_path = f"data/hoi4d_hf/hand_{args.additional_tag}_{set_label}_parquets"
