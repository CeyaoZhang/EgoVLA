...
def read_intrinsics_txt(img_instrics_path, resized_width, resized_height):
  ...
  # intrinsics[1] = intrinsics[1] * scale_y
    # print(data)
  dist_coeffs = np.array(data[9: 9 + 8])
  return intrinsics, dist_coeffs, width, height
...
def main():
  ...
  frame_rate, frame_num = get_framerate(
      os.path.join(base_path, "Video_pitchshift.mp4"))
  img_timing_array = []
  ...
  # img_pose_path = os.path.join(img_path, 'Pose_sync.txt')
  head_pose_path = os.path.join(base_path, "Head", 'Head_sync.txt')
  head_pose_array = read_pose_txt(head_pose_path)
  
  print("-" * 100)
  print(img_pose_array)
  print(head_pose_array)
  
  # Read cam instrics
  img_instrics_path = os.path.join(img_path, 'Intrinsics.txt')
  img_intrinsics, dist_coeff, width, height = read_intrinsics_txt(img_instrics_path, 454, 256)
  ...
  for frame in tqdm.tqdm(range(num_frames)):
    ...
    # print(hand_point_left)
    # print(hand_point_right)
    ...
    if args.save_video:
      ...
      # img = cv2.undistort(img, img_intrinsics, dist_coeff)
