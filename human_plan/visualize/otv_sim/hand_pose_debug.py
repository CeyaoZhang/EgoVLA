...
def read_pose_txt_np(img_pose_path):
...
def read_hand_pose_txt_np(hand_path):
  ...
  hand_array = []
  data = np.loadtxt(hand_path)

  hand_translation = data[:, 3:-52].reshape(-1, 26, 4, 4)[:, :, :, -1:]

  # -1, 26, 4, 1

  # hand_array = np.concatenate([
  #     data[:, :4], hand_translation.reshape(-1, 26 * 3)
  # ], axis=-1)
  return hand_translation
  ...
  with open(hand_path) as f:
    ...
    for line in lines:
      ...
      for line_data_reshape_elem in line_data_reshape:
        ...
        # print(line_data_reshape_elem[:3, -1] - location[:3].T[0])

      line_data_xyz = np.array(line_data_xyz).T
      # print(line_data_xyz.shape)
      hand = line_data[:4]
...
def main():
  ...
  frame_rate, frame_num = get_framerate(
      os.path.join(base_path, "Video_compress.mp4"))
  img_timing_array = []
  ...
  if args.save_video:
    ...
    if not os.path.exists(mpeg_aligned_img_path):
      ...
      for ii, img_sync_timestamp in enumerate(img_sync_timing_array[:]):
        ...
        source = os.path.join(
            mpeg_img_path, '{0:06d}.png'.format(mpeg_indices[0][0]))
  ...
  left_hand_array_np = read_hand_pose_txt_np(left_hand_path)

  # print(left_hand_array.shape, left_hand_array_np.shape)
  # print(left_hand_array[:, 4:] - left_hand_array_np[:, 4:])
  # assert np.allclose(left_hand_array, left_hand_array_np)
  # Read right hand
  right_hand_path = os.path.join(hands_path, 'Right_sync.txt')
  right_hand_array = read_hand_pose_txt(right_hand_path)
  right_hand_array_np = read_hand_pose_txt_np(right_hand_path)
  ...
  if args.save_video:
    ...
    video_out = cv2.VideoWriter(os.path.join(
        base_path, 'project.mp4'), fourcc, frame_rate, (896, 504), isColor=True)
  ...
  img_pose_array_np = read_pose_txt_np(img_pose_path)

  # assert np.allclose(img_pose_array, img_pose_array_np)
  ...
  # hand_point_left_np = left_hand_array_np[:, 4:].reshape(-1, 26, 3)
  # hand_point_right_np = right__np[:, 4:].reshape(-1, 26, 3)

  print(left_hand_array_np.shape)
  print(np.linalg.inv(img_pose_array_np)[:, np.newaxis, :, :].shape)
  print(axis_transform[np.newaxis, np.newaxis, :, :].shape)
  hand_point_left_np = axis_transform[np.newaxis, np.newaxis, :, :] @ \
    np.linalg.inv(img_pose_array_np)[:, np.newaxis, :, :] @ \
    left_hand_array_np

  # print(right_hand_array_np.shape)
  hand_point_right_np = axis_transform[np.newaxis, np.newaxis, :, :] @ \
    np.linalg.inv(img_pose_array_np)[:, np.newaxis, :, :] @ \
    right_hand_array_np
  ...
  for frame in tqdm.tqdm(range(num_frames)):
    ...
    hand_point_left_raw = left_hand_array[left_indices[0][0]][4:].reshape(
        3, -1)
    hand_point_right_raw = right_hand_array[right_indices[0][0]][4:].reshape(
        3, -1)
    ...
    hand_point_right = axis_transform @ \
      np.linalg.inv(img_pose) @ \
      np.concatenate((hand_point_right_raw, [[1] * np.shape(hand_point_right_raw)[1]]))

    # img_pose pose

    # hand_point_right_np = axis_transform @ \
    #   np.linalg.inv(img_pose) @ \
    #   np.concatenate((hand_point_right_raw, [[1] * np.shape(hand_point_right_raw)[1]]))
    right_hand_result.append(hand_point_right)
    
    # print(hand_point_right - hand_point_right_np)

    # c = np.einsum('BNi,Bi ->BN', a, b)
    hand_point_left = np.dot(axis_transform, np.dot(np.linalg.inv(
        img_pose), np.concatenate((hand_point_left_raw, [[1] * np.shape(hand_point_right_raw)[1]]))))

    left_hand_result.append(hand_point_left)

    # print(hand_point_left.shape)
    # print(hand_point_right.shape)
    ...
    if args.save_video:
      ...
      img = cv2.imread(os.path.join(
          mpeg_aligned_img_path, '{0:06d}.png'.format(frame)))
      ...
      cv2.imwrite(
          os.path.join(projected_path, '{0:06d}.png'.format(frame)), img
      )

      video_out.write(img)


  left_hand_result = np.stack(left_hand_result)
  right_hand_result = np.stack(right_hand_result)
  
  print(left_hand_result.shape)
  print(right_hand_result.shape)

  hand_point_left_np = np.transpose(np.squeeze(hand_point_left_np), (0, 2, 1))
  hand_point_right_np = np.transpose(np.squeeze(hand_point_right_np), (0, 2, 1))
  print(hand_point_left_np.shape)
  print(hand_point_right_np.shape)

  print(hand_point_left_np - left_hand_result)
  print(hand_point_right_np - right_hand_result)

  print(np.allclose(hand_point_left_np, left_hand_result))
  print(np.allclose(hand_point_right_np, right_hand_result))
  if args.save_video:
